...
function MediaLibrary() {
  ...
  useEffect(() => {
    if (!desktop) return
    let active = true
    invokeDesktop('list_media')
      .then(stored => Promise.all(stored.map(async item => ({ ...item, url: await localAssetUrl(item.path) }))))
      .then(withUrls => { if (active) setItems(withUrls) })
...
function App() {
  ...
  const today = new Intl.DateTimeFormat('vi-VN', { weekday: 'long', day: '2-digit', month: 'long' }).format(startedAt)
  const tokenExpiry = new Intl.DateTimeFormat('vi-VN').format(new Date(startedAt.getTime() + 28 * 86400000))
